...
{
   angular.module("positioning.progress", []).directive("progressBarCsv", ["flashService", "messageService", "submitService", function (flashService, messageService, submitService) {
      return {
         scope: {
            state: "="
         },
...

{
    var ProxyService = function () {
        function ProxyService($q, configService) {
        ...
        _createClass(ProxyService, [{
            ...
            value: function post(data) {
                ...
                var template = this.config.uploadTemplate;
                template += template.lastIndexOf("?") > 0 ? "" : "?"; // 


                var keyValueStrs = Object.keys(parameters).map(function (key) {
    ...
    angular.module("positioning.proxy", []).service("submitService", ProxyService);
}
"use strict";
